...
(* "power" printed once *)
(*
The code built at Line 1, characters 17-72 has hardly serializable CSPs: square at Line 5, characters 26-32

square at Line 5, characters 26-32
...
*)
let power7 : int -> int = run power7_cde;;
...
let power7 : int -> int = 
  run .<fun x -> .~(Printf.printf "power\n"; power 7 .<x>.)>.;;

(* But the following does. It is the explicit version of the above *)
let power7 : int -> int = 
  Runcode.run_bytecode (close_code ~csp:CSP_ok
...
let true = 
 try let x = ref .<1>. in
     let _ = .<fun v -> .~(x := .<v>.; .<()>.)>. in
     ignore (.<1 + .~(!x)>.); (* triggers an error with the message below *)
     false 
 with Failure e -> print_string e; true
;;
(*
...
let 8 = (run test_sgibo_CPS_only5) 1 1;;
