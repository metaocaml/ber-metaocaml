...
contained in `exp'.
Previusly genlet performed no let-insertion if the argument is a code
...
let typecode_path =
  let path =
    Path.(Pdot (Pident (Ident.create_persistent "Trx"), "code", 0)) in
...
let typepat_code_path =
  let path =
    Path.(Pdot (Pident (Ident.create_persistent "Trx"), "pat_code", 0)) in
...
let typeval_code_path =
  let path =
    Path.(Pdot (Pident (Ident.create_persistent "Trx"), "val_code", 0)) in
...
let scope_extrusion_error : 
  ...
  Format.fprintf Format.str_formatter
    ...
    Location.print detected Location.print ast.pexp_loc
...
let is_too_cheap_to_let_bind : Parsetree.expression -> bool = fun exp ->
  ...
  let scan_attrs = List.iter @@ function
  | ({txt = "metaocaml.bracket"},_) -> res := false
  | ({txt = "metaocaml.escape"},_)  -> res := false
  | _ -> () 
  in
  let defl = default_iterator in
  let iter = 
