(*
  ...
  Clearly scope extrusion cannot occur without escapes. The result
  of an escape may contain an extruded (unbound) variable.
  Further, an unbound variable may be detected when running or printing
  the code expression (which must be closed).
  So, we should run the extrusion detection check on each escape,
  and when printing or running -- essentially on each use of the code
  expression.

  The scope extrusion check should scan the Parsetree and determine
  if the code contains unbound gensymed identifiers. All identifiers
  generated by with_binding_region have the form
      Longident.Lident (name_number)
  As a matter of fact, all identifiers of that form are introduced by
