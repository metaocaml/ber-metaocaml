/* A small glue file to compile native MetaOCaml code

   All the compilation is done by the patched ocamlopt, and this program
   merely executes it.
   ...
   libraries, esp. env, are stateful).
...
static char path_ocamlopt [] = BINDIR  "/ocamlopt";
...
int main(int argc, char *const argv[],char *const envp[])
{
  ...
  execve(path_ocamlopt,new_argv,envp); /* Does not normally return */
  perror("execve failed, perhaps wrong path to ocamlopt");
  exit(2);
}
